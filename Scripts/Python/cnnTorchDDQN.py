...
camera = None        # dxcam on Windows; unused on Linux with thread-local mss
monitor = None       # unused with thread-local, but keep for clarity
...
def _ensure_linux_grabber():
    """Per-thread mss() and safe monitor rectangle."""
    if getattr(_tls, "sct", None) is None:
        import mss  
        disp = os.environ.get("DISPLAY", ":0")
        _tls.sct = mss.mss(display=disp)
        mon = _tls.sct.monitors[1]
        _tls.monitor = {"left": mon["left"], "top": mon["top"],
...
def get_screenshot():
    if sys.platform == "win32":
        ...
        _ensure_linux_grabber()
        try:
            raw = np.array(_tls.sct.grab(_tls.monitor)) 
        except Exception as e:
            print(f"[mss] grab failed ({e}); reinitializing this thread…")
            _tls.sct = None
            _ensure_linux_grabber()
            raw = np.array(_tls.sct.grab(_tls.monitor))
        bgr = raw[:, :, :3]
